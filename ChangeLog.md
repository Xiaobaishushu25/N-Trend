...
### 多端同步架构升级与服务端化（v3.0 大版本发布）

**重大架构演进**
- **服务端化拆分（n-headless & n-protocol）**：
  - 抽取核心数据、模型与计算逻辑为独立守护进程 `n-headless`，支持 Linux/Windows 无界面后台长期稳定运行；
  - 新建 `n-protocol` 统一契约包，定义前后端 REST API、WebSocket 推送协议与设备鉴权 DTO。
- **多端与移动化适配（Desktop / Mobile / Android）**：
  - 客户端架构重构为远程/本地双模式，支持多端连接远程同一中心服务端；
  - 界面全面支持响应式与触屏手势，适配移动设备视口（min 360x480）；
  - 新增 Android CI 构建工作流（`.github/workflows/android.yml`），支持全自动打包 APK。
- **灾备与数据自愈（Backup & Recovery）**：
  - `n-core` 内置数据库在线热备份、自动滚动归档与容灾恢复体系；
  - 编写生产级部署脚本与多端同步方案、运维灾备手册。
- **全栈版本号统一升级**：
  - Workspace、Tauri、Package 以及协议版本全面提升为 `3.0.0`。

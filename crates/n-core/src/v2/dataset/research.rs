...
impl Cell {
    fn add(&mut self, row: &DatasetRow) {
        self.n += 1;
        self.wins += usize::from(row.label_win == 1);
    }
    fn wr(self) -> f64 {
        if self.n == 0 {
            0.0
...
fn trend_regime(row: &DatasetRow) -> Option<&'static str> {
    ...
    if strength < ADX_THRESHOLD {
        return Some("Range/Weak");
    }
    if gap > TREND_GAP_THRESHOLD {
        Some("Strong Up")
    } else if gap < -TREND_GAP_THRESHOLD {
        Some("Strong Down")
    } else {
        Some("Range/Weak")
    }
...
fn fmt_cell(cell: Cell, prior: f64) -> String {
    if cell.n == 0 {
        ...
        format!("{} / {:.1}% / {:.3}", cell.n, cell.wr() * 100.0, prior)
    }
}
...
pub fn render_market_context_research(rows: &[DatasetRow]) -> String {
    ...
    for row in dev {
        if let Some(regime) = trend_regime(row) {
            trend
                .entry((regime, row.direction.as_str()))
                .or_default()
                .add(row);
        }
        if let Some(pos) = row.range_position_10d {
            ...
            position
                .entry((bucket, row.direction.as_str()))
                .or_default()
                .add(row);
            if let Some(regime) = trend_regime(row) {
                matrix
                    .entry((regime, row.direction.as_str(), coarse_position(pos)))
                    .or_default()
                    .add(row);
            }
        }
    ...
    out.push_str(&format!(
        "Fixed rules: trend gap threshold = {:.1} ATR, ADX threshold = {:.2}; RR excluded.\n\n",
        TREND_GAP_THRESHOLD, ADX_THRESHOLD
    ));
    out.push_str(&format!(
        "Rows: DEV {} / LOCKED_HISTORICAL_TEST {}. Average DEV baseline P = {:.3}.\n\n",
        rows.len().saturating_sub(locked.len()),
        locked.len(),
        prior
    ));
    ...
    for regime in ["Strong Up", "Range/Weak", "Strong Down"] {
        out.push_str(&format!(
            ...
            regime,
            fmt_cell(
                *trend.get(&(regime, "up")).unwrap_or(&Cell::default()),
                prior
            ),
            fmt_cell(
                *trend.get(&(regime, "down")).unwrap_or(&Cell::default()),
                prior
            )
        ));
    }
    ...
    for bucket in ["Below 10d low", "Low", "Middle", "High", "Above 10d high"] {
        out.push_str(&format!(
            ...
            bucket,
            fmt_cell(
                *position.get(&(bucket, "up")).unwrap_or(&Cell::default()),
                prior
            ),
            fmt_cell(
                *position.get(&(bucket, "down")).unwrap_or(&Cell::default()),
                prior
            )
        ));
    }
    ...
    for fold in folds.iter().filter(|f| f.valid_end > f.valid_start) {
        ...
        let mut up_low = Cell::default();
        let mut up_high = Cell::default();
        let mut down_high = Cell::default();
        let mut down_low = Cell::default();
        for row in slice {
            if let Some(p) = row.range_position_10d {
                match (row.direction.as_str(), coarse_position(p)) {
                    ("up", "Low") => up_low.add(row),
                    ("up", "High") => up_high.add(row),
                    ("down", "High") => down_high.add(row),
                    ("down", "Low") => down_low.add(row),
                    _ => {}
                }
            }
        }
        if up_low.n > 0 && up_high.n > 0 && up_low.wr() > up_high.wr() {
            h2 += 1;
        }
        if down_high.n > 0 && down_low.n > 0 && down_high.wr() > down_low.wr() {
            h2 += 1;
        }
    }

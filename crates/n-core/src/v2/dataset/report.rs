...
pub fn missing_report(rows: &[DatasetRow]) -> MissingReport {
    ...
    let mut dropped = 0;
    for r in rows {
        if r.warning_volume_ratio.is_none() {
            *counts.entry("warning_volume_ratio".into()).or_insert(0) += 1;
        }
        if r.trigger_volume_ratio.is_none() {
            *counts.entry("trigger_volume_ratio".into()).or_insert(0) += 1;
        }
        if r.trigger_close_overshoot_r.is_none() {
            *counts
                .entry("trigger_close_overshoot_r".into())
                .or_insert(0) += 1;
        }
    ...
    for (k, v) in &counts {
        rate.insert(k.clone(), *v as f64 / total.max(1) as f64);
    }
    MissingReport {
        total,
        missing_counts: counts,
        dropped_due_to_atr: dropped,
        missing_rate: rate,
    }
}
...
pub fn distribution_reports(rows: &[DatasetRow]) -> Vec<DistributionReport> {
    ...
    let feats: Vec<(&str, Vec<f64>)> = vec![
        ...
        (
            ...
            rows.iter()
                .filter_map(|r| r.trigger_close_overshoot_r)
                .collect(),
        ),
        (
            "trigger_close_location",
            rows.iter()
                .filter_map(|r| r.trigger_close_location)
                .collect(),
        ),
    ];

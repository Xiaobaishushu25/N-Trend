...
pub fn normalize_direction(setup: &mut SetupFeatures, trigger: Option<&mut TriggerFeatures>) {
    if setup.direction == "down" {
        ...
        if let Some(t) = trigger {
            if let Some(v) = t.close_overshoot_r {
                t.close_overshoot_r = Some(-v);
            }
            // close_location is mirrored: 1 - loc (high becomes low)
            if let Some(v) = t.close_location {
                t.close_location = Some(1.0 - v);
            }
...
mod tests {
    ...
    fn normalize_flips_overshoot() {
        let mut s = SetupFeatures {
            ...
            a_strong_count: 2,
            setup_quality: 3.5,
            trend60_state: "up".into(),
            warning_close_location: Some(0.2),
            warning_body_atr: Some(0.5),
            warning_wick_ratio: Some(1.0),
            warning_volume_ratio: Some(1.2),
            normalized: false,
            missing_mask: 0,
        };
        let mut t = TriggerFeatures {
            trigger_bar_ts: "2024-01-01 10:15:00".into(),
            close_overshoot_r: Some(0.5),

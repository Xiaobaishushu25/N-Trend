...
pub fn predict_logistic(bundle: &InferenceBundle, row: &DatasetRow) -> Option<Prediction> {
    ...
    let contribs = m
        ...
        .map(|c| Contribution {
            feature: c.feature,
            value: c.value,
            coefficient: Some(c.coefficient),
...
mod tests {
    ...
    fn mk_row() -> DatasetRow {
        DatasetRow {
            ...
            exit_ts: Some("2024-01-02 10:00:00".into()),
    ...
    fn predict_logistic_smoke() {
        ...
        m.scaler_means = vec![0.0];
        m.scaler_stds = vec![1.0];
        m.coefficients = vec![0.5];
        m.intercept = 0.0;
        let bundle = InferenceBundle {
            model_id: "logistic-v1".into(),
            feature_whitelist: vec!["a_move_atr".into()],
            scaler_means: vec![0.0],
            scaler_stds: vec![1.0],
            logistic: Some(m),
            gam: None,
            schema_version: "v2.1".into(),

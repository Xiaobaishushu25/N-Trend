...
impl SplineTable {
    ...
    pub fn eval(&self, x: f64) -> f64 {
        ...
        if x >= self.knots[self.knots.len() - 1] {
            return self.values[self.knots.len() - 1];
        }
...
impl GamModel {
    ...
    pub fn predict_logit(&self, row: &DatasetRow) -> f64 {
        ...
        for spl in &self.splines {
            if let Some(v) = get_feature(row, &spl.feature) {
                s += spl.eval(v);
            }
        }
        s
    ...
    pub fn contributions(&self, row: &DatasetRow) -> Vec<GamContribution> {
        ...
        for spl in &self.splines {
            if let Some(v) = get_feature(row, &spl.feature) {
                out.push(GamContribution {
                    feature: spl.feature.clone(),
                    value: v,
                    contribution: spl.eval(v),
                    kind: "spline".into(),
                });
            }
        }
    ...
    fn default() -> Self {
        Self {
            ...
            spline_features: vec![
                "a_move_atr".into(),
                "retracement".into(),
                "trigger_close_overshoot_r".into(),
                "trigger_close_location".into(),
            ],
        }
    }
}
...
pub fn train_gam(rows: &[DatasetRow], cfg: &GamTrainConfig) -> (GamModel, Metrics) {
    ...
    let mut model = GamModel {
        intercept,
        splines: vec![],
        linear_coefficients: vec![0.0; cfg.linear_features.len()],
        linear_features: cfg.linear_features.clone(),
    };
        ...
        for (k_idx, _knot) in knots.iter().enumerate() {
            ...
            for r in rows {
                if let Some(v) = get_feature(r, feat) {
                    ...
                    for (i, kk) in knots.iter().enumerate() {
                        let d = (v - kk).abs();
                        if d < bestd {
                            bestd = d;
                            best = i;
            ...
            if bin_rows.is_empty() {
                ...
                let wr = bin_rows.iter().filter(|r| r.label_win == 1).count() as f64
                    / bin_rows.len() as f64;
    ...
    vec![
        ...
        (
            ...
            vec![
                "trigger_close_overshoot_r".into(),
                "trigger_close_location".into(),
                "trigger_body_atr".into(),
                "trigger_wick_atr".into(),
                "internal_swing_margin_r".into(),
                "chase_distance_r".into(),
            ],
        ),
    ...
    fn gam_train_smoke() {
        ...
        let rows: Vec<DatasetRow> = (0..20)
            .map(|i| DatasetRow {
                ...
                a_move: 10.0,
                b_move: 5.0,
                a_move_atr: if i < 10 { 1.0 } else { 5.0 },
                b_move_atr: 1.0,
                a_speed: 1.0,
                retracement: 0.5,
                ...
                missing_mask: 0,
                label_win: if i < 10 { 0 } else { 1 },
                r_multiple: Some(1.0),
                is_1r_aux_win: Some(true),
                trigger_bar_ts: Some(format!("2024-01-{:02} 10:00:00", i + 1)),
                exit_ts: Some("2024-01-20 11:00:00".into()),

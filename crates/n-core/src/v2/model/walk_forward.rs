...
pub fn walk_forward(n: usize, n_splits: usize) -> Vec<Fold> {
    ...
    folds
        .into_iter()
        .filter(|f| f.train_end - f.train_start >= 10 && f.valid_end > f.valid_start)
        .collect()
}
...
pub fn walk_forward_purge_aware(rows: &[DatasetRow], n_splits: usize) -> Vec<Fold> {
    ...
    for f in folds.iter_mut() {
        if f.train_end == 0 || f.valid_start >= rows.len() {
            continue;
        }
        // if same timestamp straddles boundary, expand train to include all rows with that timestamp
        let mut vs = f.valid_start;
        let train_last_ts = rows
            .get(f.train_end.saturating_sub(1))
            .and_then(|r| r.trigger_bar_ts.as_deref())
            .unwrap_or("");
        ...
        while f.valid_end < rows.len() {
            ...
            let next_ts = rows
                ...
                .and_then(|r| r.trigger_bar_ts.as_deref())
                .unwrap_or("");
            if !cur_ts.is_empty() && cur_ts == next_ts {
                f.valid_end += 1;
            } else {
                break;
            }
        ...
        if f.valid_start < rows.len() {
            if let Some(valid_ts) = rows[f.valid_start].trigger_bar_ts.as_deref() {
                ...
                if let Some(idx) = first_overlapping {
                    f.train_end = idx;
                }
            }
        }
    ...
    folds
        .into_iter()
        .filter(|f| f.valid_start < f.valid_end && f.train_end - f.train_start >= 5)
        .collect()
}
...
pub fn assert_purge(rows: &[DatasetRow], folds: &[Fold]) -> Result<()> {
    for f in folds {
        ...
        let train_last = rows[f.train_end - 1]
            .trigger_bar_ts
            .as_deref()
            .unwrap_or("");
        ...
        if !train_last.is_empty() && !valid_first.is_empty() && train_last >= valid_first {
            return Err(anyhow!(
                "purge violated: train_last {} >= valid_first {} in fold {:?}",
                train_last,
                valid_first,
                f
            ));
        ...
        for row in &rows[f.train_start..f.train_end] {
            let label_end = row
                .exit_ts
                .as_deref()
                .or(row.trigger_bar_ts.as_deref())
                .unwrap_or("");
            if !label_end.is_empty() && !valid_first.is_empty() && label_end >= valid_first {
                return Err(anyhow!(
                    "label horizon overlaps validation: exit {} >= valid {} in fold {:?}",
                    label_end,
                    valid_first,
                    f
                ));
...
pub fn split_final_holdout<'a>(
    rows: &'a [DatasetRow],
    holdout_size: usize,
) -> (&'a [DatasetRow], &'a [DatasetRow]) {
...
mod tests {
    ...
    fn walk_forward_basic() {
        let rows: Vec<DatasetRow> = (0..100)
            .map(|i| mk_row(&format!("2024-01-{:02} 10:00:00", (i % 28) + 1)))
            .collect();
    ...
    fn purge_fails_on_overlap() {
        let rows: Vec<DatasetRow> =
            vec![mk_row("2024-01-02 10:00:00"), mk_row("2024-01-01 10:00:00")]; // unsorted
        let folds = vec![Fold {
            train_start: 0,
            train_end: 1,
            valid_start: 1,
    ...
    fn purge_reduces_training_end_for_overlapping_label() {
        ...
        assert!(folds
            .iter()
            .all(|f| assert_purge(&rows, std::slice::from_ref(f)).is_ok()));
    }
}

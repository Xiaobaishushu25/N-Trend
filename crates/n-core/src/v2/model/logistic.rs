...
use crate::v2::model::scaler::{get_feature, StandardScaler};
...
impl LogisticModel {
    pub fn new(feature_names: Vec<String>) -> Self {
        ...
        Self {
            ...
            scaler_means: vec![0.0; n],
            scaler_stds: vec![1.0; n],
        }
    }
    pub fn logit(&self, features: &[f64]) -> f64 {
        ...
        for (c, f) in self.coefficients.iter().zip(features.iter()) {
            s += c * f;
        }
        s
    }
    ...
    pub fn feature_contributions(&self, row: &DatasetRow) -> Vec<FeatureContribution> {
        ...
        for (j, name) in self.feature_names.iter().enumerate() {
            ...
            let sv = v
                .map(|value| (value - self.scaler_means[j]) / self.scaler_stds[j].max(1e-9))
                .unwrap_or(0.0);
...
pub fn predict_p(model: &LogisticModel, features: &[f64]) -> f64 {
    model.predict_p(features)
}
...
pub fn train(
    ...
    let prior = (y_train.iter().filter(|y| **y == 1).count() as f64 / y_train.len() as f64)
        .clamp(0.05, 0.95);
    ...
    for epoch in 0..cfg.epochs {
        ...
        // L2 penalty
        for j in 0..d {
            _loss += 0.5 * cfg.l2 * model.coefficients[j].powi(2) / n;
        }
        for j in 0..d {
            grad_coef[j] = grad_coef[j] / n + cfg.l2 * model.coefficients[j] / n;
        }
        grad_intercept /= n;
        // step
        model.intercept -= cfg.lr * grad_intercept;
        for j in 0..d {
            model.coefficients[j] -= cfg.lr * grad_coef[j];
        }
        if cfg.verbose && epoch % 100 == 0 {
            // tracing::debug!("epoch {} loss {:.4}", epoch, loss);
        }
        // early stop if grad small
        if grad_coef.iter().map(|g| g.abs()).fold(0.0, f64::max) < 1e-7
            && grad_intercept.abs() < 1e-7
        {
            break;
        }
    }
...
mod tests {
    ...
    fn mk_row(a_atr: f64, label: i32, ts: &str) -> DatasetRow {
        DatasetRow {
            ...
            trigger_bar_ts: Some(ts.into()),
            exit_ts: Some("2025-01-02 10:00:00".into()),
    ...
    fn train_separates() {
        let rows: Vec<DatasetRow> = (0..20)
            .map(|i| {
                mk_row(
                    if i < 10 { 1.0 } else { 5.0 },
                    if i < 10 { 0 } else { 1 },
                    &format!("2024-01-{:02} 10:00:00", i + 1),
                )
            })
            .collect();
    ...
    fn missing_inference_matches_training_transform() {
        let rows = vec![
            mk_row(1.0, 0, "2024-01-01 10:00:00"),
            mk_row(3.0, 1, "2024-01-02 10:00:00"),
        ];

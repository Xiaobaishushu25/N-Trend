...
pub fn calibration(y_true: &[i32], p_pred: &[f64], n_bins: usize) -> Vec<CalibrationBucket> {
    ...
    buckets
        .into_iter()
        .enumerate()
        .map(|(i, (c, sum_p, sum_y))| CalibrationBucket {
            bin: i,
            count: c,
            avg_p: if c > 0 { sum_p / c as f64 } else { 0.0 },
            avg_y: if c > 0 { sum_y / c as f64 } else { 0.0 },
        })
        .collect()
}
...
pub fn top20_lift(y_true: &[i32], p_pred: &[f64]) -> f64 {
    ...
    let mut pairs: Vec<(f64, i32)> = y_true
        .iter()
        .zip(p_pred.iter())
        .map(|(y, p)| (*p, *y))
        .collect();
...
fn tail_win_rate(y_true: &[i32], p_pred: &[f64], top: bool) -> f64 {
    ...
    let k = ((pairs.len() as f64 * 0.1).ceil() as usize)
        .max(1)
        .min(pairs.len());
    ...
    (0..10)
        .map(|bin| {
            ...
            CalibrationBucket {
                bin,
                count: part.len(),
                avg_p: part.iter().map(|(p, _)| *p).sum::<f64>() / part.len() as f64,
                avg_y: part.iter().map(|(_, y)| *y as f64).sum::<f64>() / part.len() as f64,
            }
        })
        .collect()
}
...
pub fn compute_metrics(y_true: &[i32], p_pred: &[f64]) -> Metrics {
    let observed_prior = if y_true.is_empty() {
        0.5
    } else {
...
pub fn compute_metrics_with_baseline(
    y_true: &[i32],
    p_pred: &[f64],
    baseline_prior: f64,
) -> Metrics {
    ...
    let win_rate = if n > 0 {
        baseline_prior.clamp(1e-6, 1.0 - 1e-6)
    } else {
        0.5
    };
    ...
    Metrics {
        ...
        baseline_brier,
        baseline_logloss,
        brier_skill: if baseline_brier > 1e-12 {
            1.0 - brier / baseline_brier
        } else {
            0.0
        },
        top20_lift: lift,
        top10_win_rate: tail_win_rate(y_true, p_pred, true),
        bottom10_win_rate: tail_win_rate(y_true, p_pred, false),
        constant_win_rate: win_rate,
        calibration_slope,
...
mod tests {
    ...
    fn brier_perfect_is_zero() {
        assert!((brier_score(&[1, 0], &[1.0, 0.0]) - 0.0).abs() < 1e-9);
    }
    #[test]
    fn auc_perfect_is_one() {
        assert!((auc(&[1, 1, 0, 0], &[0.9, 0.8, 0.3, 0.2]) - 1.0).abs() < 1e-9);
    }
    #[test]
    fn auc_random_half() {
        let a = auc(&[1, 0, 1, 0], &[0.5, 0.5, 0.5, 0.5]);
        assert!((a - 0.5).abs() < 1e-9);
    }
    #[test]
    fn logloss_clamped() {
        let ll = logloss(&[1], &[0.0]);
        assert!(ll < 40.0);
    }
    #[test]

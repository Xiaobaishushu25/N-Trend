...
use crate::v2::features::{
    extract_market_context, extract_trigger_features, MarketContextSnapshot, SetupFeatures,
    TriggerFeatures,
};
use crate::v2::{EXECUTION_VERSION, FEATURE_SCHEMA_VERSION, PATTERN_LOGIC_VERSION};
use anyhow::Result;
...
impl ReplayEngine {
    ...
    pub fn replay_history_with_rollovers(
        ...
        for candidate in candidates {
            ...
            let warning_extreme = if is_long {
                warning_bar.high
            } else {
                warning_bar.low
            };
            let s0_price = if is_long {
                bars15[candidate.s0_index].low
            } else {
                bars15[candidate.s0_index].high
            };
            let s1_price = if is_long {
                bars15[candidate.s1_index].high
            } else {
                bars15[candidate.s1_index].low
            };
            let s2_price = if is_long {
                bars15[candidate.s2_index].low
            } else {
                bars15[candidate.s2_index].high
            };
            ...
            let sf = SetupFeatures {
                ...
                a_speed: if candidate.a_bars > 0 {
                    candidate.a_move / candidate.a_bars as f64
                } else {
                    0.0
                },
                a_move_atr: if a_atr > 1e-9 {
                    candidate.a_move / a_atr
                } else {
                    0.0
                },
                b_move_atr: if b_atr > 1e-9 {
                    candidate.b_move / b_atr
                } else {
                    0.0
                },
                ...
                warning_wick_ratio: Some(if warning_body > 1e-9 {
                    warning_upper.max(warning_lower) / warning_body
                } else {
                    0.0
                }),
                warning_volume_ratio: crate::analyze::outcome::vol_ratio_at(
                    &bars15,
                    candidate.warning_index,
                ),
                normalized: false,
                missing_mask: 0,
            };
            ...
            events.push(ReplayEvent {
                ...
                grade: candidate.grade.clone(),
                level: candidate.level.to_string(),
                s0_ts: bars15[candidate.s0_index].dt.to_bar_ts(),
                s1_ts: bars15[candidate.s1_index].dt.to_bar_ts(),
                s2_ts: bars15[candidate.s2_index].dt.to_bar_ts(),
                s0_price,
                s1_price,
                s2_price,
                a_move: candidate.a_move,
                ...
                entry_price: None,
                outcome: None,
                schema_version: FEATURE_SCHEMA_VERSION.into(),
                market_context: None,
                pattern_version: PATTERN_LOGIC_VERSION.into(),
        ...
        for ev in events.iter_mut() {
            let warn_idx_opt = bars15
                .iter()
                .position(|b| bar_ts_matches(&b.dt.to_bar_ts(), &ev.warning_ts));
            let s2_idx_opt = bars15
                .iter()
                .position(|b| bar_ts_matches(&b.dt.to_bar_ts(), &ev.s2_ts));
            let Some(start_idx) = warn_idx_opt.or(s2_idx_opt) else {
                continue;
            };
            ...
            if let Some(ti) = trigger_idx {
                ...
                let atr = crate::analyze::indicators::atr(&bars15, 20)
                    .get(ti)
                    .and_then(|x| *x);
                ...
                if ti > 0 && !bars15[ti - 1].rollover {
                    ...
                    let gap = if is_long {
                        prev_close < ev.entry && cur_open > ev.entry
                    } else {
                        prev_close > ev.entry && cur_open < ev.entry
                    };
                    if gap {
                ...
                for k in ti..look {
                    ...
                    let mfe_contrib = if is_long {
                        (b.high - fill) / risk
                    } else {
                        (fill - b.low) / risk
                    };
                    let mae_contrib = if is_long {
                        (b.low - fill) / risk
                    } else {
                        (fill - b.high) / risk
                    };
                    ...
                    if let Some(mut grade) = trail_grade {
                        loop {
                            ...
                            let next_price = if is_long {
                                fill + trail_r(next_grade) * risk
                            } else {
                                fill - trail_r(next_grade) * risk
                            };
                            let next_hit = if is_long {
                                b.high >= next_price
                            } else {
                                b.low <= next_price
                            };
                        ...
                        let trail_price = if is_long {
                            fill + trail_r(grade) * risk
                        } else {
                            fill - trail_r(grade) * risk
                        };
                        let fell_back = if is_long {
                            b.low <= trail_price
                        } else {
                            b.high >= trail_price
                        };
                        if fell_back {
                            exit = Some((k, trail_price, "target"));
                            break;
                        }
                    }
                    if k - ti + 1 >= crate::analyze::outcome::NO_FOLLOW_BAR
                        && mfe < crate::analyze::outcome::NO_FOLLOW_MFE_R
                    {
                ...
                if let Some((ei, ep, reason)) = exit {
                    ...
                    ev.outcome = Some(ReplayOutcome {
                        ...
                        exit_ts: bars15[ei].dt.to_bar_ts(),
                        exit_price: ep,
                        r_multiple: r_mult,
                        mfe_r: Some(mfe),
                        mae_r: Some(mae),
                        is_1r_aux_win: Some(mfe >= 1.0),
                    });
                }
            }
        }
...
fn trail_r(grade: usize) -> f64 {
    if grade == 1 {
        ...
        crate::analyze::outcome::TRAIL_STEP_R * grade as f64
    }
}
...
fn resolve_intrabar_exit(
    raw5m: &[Kline],
    bucket: &crate::analyze::model::DT,
    stop: f64,
    target: f64,
    is_long: bool,
) -> IntrabarExit {
    for k in raw5m {
        let Some(dt) = crate::analyze::model::DT::from_bar_ts(&k.datetime) else {
            continue;
        };
        if dt.year != bucket.year
            || dt.month != bucket.month
            || dt.day != bucket.day
            || dt.hour != bucket.hour
            || dt.minute / 15 != bucket.minute / 15
        {
            continue;
        }
        let hit_stop = if is_long {
...
fn kline_to_bar(klines: &[Kline]) -> Vec<Bar> {
    klines
        .iter()
        .map(|k| {
            let dt = crate::analyze::model::DT::from_bar_ts(&k.datetime).unwrap_or(
                crate::analyze::model::DT {
                    year: 2024,
                    month: 1,
                    day: 1,
                    hour: 0,
...
mod tests {
    ...
    fn same_15m_bar_is_resolved_by_5m_order() {
        ...
        let raw = vec![
            kline("2025-01-01 10:00:00", 101.0, 99.0),
            kline("2025-01-01 10:05:00", 102.0, 100.5),
        ];
        assert_eq!(
            resolve_intrabar_exit(&raw, &bucket, 99.5, 101.5, true),
            IntrabarExit::Stop
        );
        assert_eq!(
            resolve_intrabar_exit(&raw, &bucket, 98.5, 100.5, true),
            IntrabarExit::Target
        );
    }

use anyhow::Result;
use n_core::storage;
use n_core::v2::dataset::{builder::DatasetRow, DatasetBuilder};
...
async fn main() -> Result<()> {
    ...
    while i < args.len() {
        if args[i] == "--symbol" && i + 1 < args.len() {
            symbol_arg = args[i + 1].clone();
            i += 2;
            continue;
        }
        if args[i].starts_with("--symbol=") {
            symbol_arg = args[i].splitn(2, '=').nth(1).unwrap_or("all").to_string();
            i += 1;
            continue;
        }
        if args[i] == "--from" && i + 1 < args.len() {
            from_arg = Some(args[i + 1].clone());
            i += 2;
            continue;
        }
        if args[i].starts_with("--from=") {
            from_arg = Some(
                args[i]
                    .splitn(2, '=')
                    .nth(1)
    ...
    println!(
        "V2 Dataset Builder — db={} symbol={} from={:?} paranoid={}",
        db_path,
        if symbol_all {
            "all".to_string()
        } else {
    ...
    let mut real_events_total = 0usize;
    {
        ...
        if c > 0 {
            ...
            for sym in &symbols {
                ...
                let mut raw5m: Vec<n_core::fetch::kline::Kline> =
                    Vec::with_capacity(kline_rows.len());
                for r in kline_rows {
            ...
            println!(
                "real replay events total: {} symbols {}",
                real_events_total,
                symbols.len()
            );
            ...
            let (kept, dropped) =
                n_core::v2::dataset::DatasetBuilder::filter_missing(all_rows.clone());
            if dropped > 0 {
                println!("missing filter dropped {} rows", dropped);
                all_rows = kept;
            }
        }
    }
    let rows: Vec<DatasetRow> = if !all_rows.is_empty() {
        all_rows
    } else {
        ...
        let _ = engine
            .replay_history("RB2501", &synthetic_raw, 1.0)
            .unwrap();
    ...
    println!(
        "Missing report: total={} dropped={}",
        report.total, report.dropped_due_to_atr
    );
    ...
    std::fs::write(
        &parquet_path,
        format!("{{\"hash\":\"{}\",\"rows\":{}}}", hash.0, rows.len()),
    )?;
    println!(
        "Wrote {} and {}",
        jsonl_path.display(),
    ...
    std::fs::write(
        out_dir.join("market_context_research.md"),
        n_core::v2::dataset::render_market_context_research(&rows),
    )?;

...
use n_core::v2::model::{
    assert_purge, compute_metrics_with_baseline, split_final_holdout, train_logistic,
    walk_forward_purge_aware, InferenceBundle, TrainConfig,
};
use n_core::v2::replay::{ReplayConfig, ReplayEngine};
...
fn synth_rows(n: usize) -> Vec<DatasetRow> {
    (0..n)
        .map(|i| {
            ...
            let label = if a_atr > 3.0 && overshoot > 0.3 {
                1
            } else if i % 5 == 0 {
                1
            } else {
            ...
            DatasetRow {
                ...
                setup_quality: 3.0 + (i % 4) as f64 * 0.4,
                a_move: 10.0,
                b_move: 5.0,
                a_move_atr: a_atr,
                b_move_atr: 1.8,
                a_speed: 1.2,
                retracement: ret,
                warning_volume_ratio: Some(1.0 + (i % 3) as f64 * 0.4),
                trigger_close_overshoot_r: Some(overshoot),
                trigger_close_location: Some(0.6 + (i % 5) as f64 * 0.05),
                trigger_body_atr: Some(0.9),
                trigger_volume_ratio: Some(1.1),
                trigger_wick_atr: Some(0.2),
                internal_swing_margin_r: Some(0.3),
                chase_distance_r: Some(0.2),
                missing_mask: 0,
                label_win: label,
                r_multiple: Some(if label == 1 { 1.5 } else { -1.0 }),
                is_1r_aux_win: Some(label == 1),
                trigger_bar_ts: Some(format!(
                    "2024-01-{:02} {:02}:15:00",
                    (i / 24) % 28 + 1,
                    (i % 24)
                )),
                ...
                schema_version: n_core::v2::FEATURE_SCHEMA_VERSION.into(),
                trend_gap_60: None,
                trend_slope_60: None,
...
async fn main() -> Result<()> {
    ...
    let db_path = args
        ...
        .map(|s| s.trim_start_matches("--db=").to_string())
        .unwrap_or_else(|| "ntrend.db".to_string());
    let from = args
        .iter()
        .find(|a| a.starts_with("--from="))
        .map(|s| s.trim_start_matches("--from=").to_string());
    ...
    let symbol_filter = args
        .iter()
        .find(|a| a.starts_with("--symbol="))
        .map(|s| s.trim_start_matches("--symbol=").to_string());
    println!(
        "V2 Train — db={} from={:?} paranoid={} smoke_synth={} evaluate_locked={} symbol={:?}",
        db_path, from, paranoid, smoke_synth, evaluate_locked, symbol_filter
    );
    ...
    let kline_cnt: i64 = db
        .query_one(Statement::from_string(
            DbBackend::Sqlite,
            "SELECT COUNT(*) as c FROM klines".to_string(),
        ))
        .await
        .ok()
        .flatten()
    ...
    if kline_cnt > 0 {
        let symbols: Vec<String> = if let Some(sf) = symbol_filter.clone() {
            if sf == "all" {
                vec![]
            } else {
                vec![sf]
        ...
        let symbols_to_run: Vec<String> = if !symbols.is_empty() {
            symbols
        } else {
            ...
            if syms.is_empty() {
                if let Ok(rows_db) = db
                    .query_all(Statement::from_string(
                        DbBackend::Sqlite,
                        "SELECT DISTINCT symbol FROM klines WHERE timeframe='5m' AND source='raw'"
                            .to_string(),
                    ))
                    .await
                {
                    for r in rows_db {
                        if let Ok(s) = r.try_get::<String>("", "symbol") {
                            syms.push(s);
                        }
                    }
                }
        ...
        println!(
            "symbols to replay: {:?} ({} total)",
            symbols_to_run.iter().take(5).collect::<Vec<_>>(),
            symbols_to_run.len()
        );
        ...
        for sym in &symbols_to_run {
            let symbol_meta = db
                .query_one(Statement::from_string(
                    DbBackend::Sqlite,
                    format!(
                        "SELECT tick_size, variety FROM symbols WHERE code='{}'",
                        sym.replace("'", "''")
                    ),
                ))
                .await
                .ok()
                .flatten();
            let stored_tick = symbol_meta
                .as_ref()
                .and_then(|r| r.try_get::<f64>("", "tick_size").ok())
                .unwrap_or(0.0);
            let variety = symbol_meta
                .as_ref()
                .and_then(|r| r.try_get::<String>("", "variety").ok())
                .unwrap_or_default();
            ...
            for r in kline_rows {
                ...
                raw5m.push(n_core::fetch::kline::Kline {
                    datetime: ts,
                    open,
                    high,
                    low,
            ...
            let rollover_records = rollover_rows
                .into_iter()
                .filter_map(|r| {
                    ...
                    Some(n_core::derive::rollover::RolloverRecord {
                        symbol: sym.clone(),
                        ts: r.try_get("", "ts").ok()?,
                        from_contract: r.try_get("", "from_contract").ok()?,
                        to_contract: r.try_get("", "to_contract").ok()?,
                        confirmed,
                    })
                })
                .collect::<Vec<_>>();
            if let Ok(evts) =
                engine.replay_history_with_rollovers(sym, &raw5m, tick, &rollover_records)
            {
        ...
        println!(
            "real replay events total: {} across {} symbols",
            all_events.len(),
            symbols_to_run.len()
        );
        let trig_cnt = all_events
            .iter()
            .filter(|e| e.trigger_features.is_some())
            .count();
        let out_cnt = all_events.iter().filter(|e| e.outcome.is_some()).count();
        println!(
            "diagnostic trig {} out {} both {}",
            trig_cnt,
            out_cnt,
            all_events
                .iter()
                .filter(|e| e.trigger_features.is_some() && e.outcome.is_some())
                .count()
        );
        if trig_cnt > 0 {
            for e in all_events
            ...
            {
                println!(
                    "sample trig {} {} trig_ts {:?} outcome {:?}",
                    e.symbol,
                    e.s2_ts,
                    e.trigger_bar_ts,
                    e.outcome.as_ref().map(|o| &o.outcome)
                );
            }
        } else if !all_events.is_empty() {
            for e in all_events.iter().take(2) {
                println!(
                    "sample no-trig {} dir {} s2 {} lvl {} entry {} stop {} target {}",
                    e.symbol, e.direction, e.s2_ts, e.trigger_level, e.entry, e.stop, e.target
                );
            }
        }
        ...
        if rows.is_empty() {
            if smoke_synth {
                println!(
                    "WARN: real klines produced 0 trainable rows — using explicit synth smoke mode"
                );
                rows = synth_rows(500);
            } else {
    ...
    if paranoid {
        n_core::v2::dataset::leakage::assert_no_leakage(&rows).unwrap();
    }
    ...
    if locked_test.is_empty() || dev_rows.len() < 40 {
        bail!(
            "insufficient rows for DEV plus locked historical test: total={}, dev={}, locked={}",
            rows.len(),
            dev_rows.len(),
            locked_test.len()
        );
    ...
    if !folds.is_empty() {
        assert_purge(dev_rows, &folds).unwrap();
        println!("walk-forward folds: {:?}", folds);
    }
    println!(
        "split: DEV {} LOCKED_HISTORICAL_TEST {} (fixed holdout=300)",
        dev_rows.len(),
        locked_test.len()
    );
    ...
    println!(
        "Logistic train AUC {:.3} Brier {:.3} logloss {:.3} lift {:.2}",
        log_out.metrics_train.auc,
        log_out.metrics_train.brier,
        log_out.metrics_train.logloss,
        log_out.metrics_train.top20_lift
    );
    if let Some(vm) = &log_out.metrics_valid {
        println!(
            "Logistic valid AUC {:.3} Brier {:.3} lift {:.2} vs baseline Brier {:.3}",
            vm.auc, vm.brier, vm.top20_lift, vm.baseline_brier
        );
    }
    ...
    println!(
        ...
        if evaluate_locked {
            "evaluated by explicit --evaluate-locked"
        } else {
            "RESERVED (not evaluated before feature/model selection)"
        }
    );
    ...
    let log_metrics_json = serde_json::to_string(
        &log_out
            .metrics_valid
            .as_ref()
            .unwrap_or(&log_out.metrics_train),
    )
    .unwrap();
    ...
    let log_bundle = InferenceBundle {
        model_id: log_model_id.clone(),
        feature_whitelist: whitelist.clone(),
        scaler_means: log_out.model.scaler_means.clone(),
        scaler_stds: log_out.model.scaler_stds.clone(),
        logistic: Some(log_out.model.clone()),
        gam: None,
        schema_version: n_core::v2::FEATURE_SCHEMA_VERSION.into(),
    };
    std::fs::write(
        out_dir.join("logistic_bundle.json"),
        serde_json::to_string_pretty(&log_bundle).unwrap(),
    )
    .unwrap();
    ...
    md.push_str(&format!(
        "- Train: AUC {:.3} Brier {:.3} LogLoss {:.3} Top20 lift {:.2} (baseline Brier {:.3})\n",
        log_out.metrics_train.auc,
        log_out.metrics_train.brier,
        log_out.metrics_train.logloss,
        log_out.metrics_train.top20_lift,
        log_out.metrics_train.baseline_brier
    ));
    if let Some(vm) = &log_out.metrics_valid {
    ...
    let lift_ok = log_out
        .metrics_valid
        .as_ref()
        .map(|m| m.top20_lift > 1.0)
        .unwrap_or(true);
    md.push_str(&format!(
        "\n- Acceptance: lift>1.0 is {} ({}), leakage PASS, hash reproducible PASS\n",
        if lift_ok { "PASS" } else { "WARN" },
        log_out
            .metrics_valid
            .as_ref()
            .map(|m| format!("{:.2}", m.top20_lift))
            .unwrap_or_else(|| "n/a".into())
    ));
    if rows.iter().any(|r| r.event_id.starts_with("synth")) {
        md.push_str("- Data: SYNTH smoke (DB empty or 0 trainable rows)\n");
    } else {
        md.push_str(&format!(
            "- Data: REAL replay (events {}, kline_cnt {})\n",
            real_events, kline_cnt
        ));
    }
    std::fs::write(out_dir.join("logistic_report.md"), &md).unwrap();
    std::fs::write(
        out_dir.join("gam_report.md"),
...
fn evaluate_walk_forward(
    ...
    for f in folds {
        ...
        if train_rows.len() < 40 || valid_rows.len() < 20 {
            continue;
        }
        let train_prior =
            train_rows.iter().filter(|r| r.label_win == 1).count() as f64 / train_rows.len() as f64;

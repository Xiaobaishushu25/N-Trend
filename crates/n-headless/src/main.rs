...
fn log_filter(level: &str) -> tracing_subscriber::EnvFilter {
    ...
    let base = format!(
        "{level},sqlx=warn,sea-orm=warn,sea_orm=warn,hyper=warn,reqwest=warn,rustls=warn,h2=warn"
    );
...
fn resolve_data_dir() -> PathBuf {
    ...
    if let Ok(v) = std::env::var("NTREND_DATA_DIR") {
        if !v.trim().is_empty() {
            return PathBuf::from(v);
        }
    }
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing::info!(
        "n-headless v{} 启动 | 数据目录: {}",
        env!("CARGO_PKG_VERSION"),
        data_dir.display()
    );
    if !db_path.exists() {
        tracing::warn!(
            "未找到 {}，将新建空库。请把本地 ntrend.db 复制到此路径",
            db_path.display()
        );
    }
    ...
    if std::env::args().any(|a| a == "--sentinel-eval") {
        let obs = n_core::storage::repo::load_all_observations(&db).await?;
        let trials = n_core::storage::repo::load_all_finality_trials(&db).await?;
        let default_sentinels: Vec<String> = n_core::finality::DEFAULT_SENTINELS
            .iter()
            .map(|s| s.to_string())
            .collect();
        ...
        println!(
            "总批次数: {} | 有效全哨兵批次: {}",
            res.total_batches, res.valid_batches
        );
        println!(
            "存在其他品种晚修订的批次数: {} (批次误判率: {:.2}%)",
            res.batches_with_non_sentinel_late_revision,
            res.batch_false_final_rate * 100.0
        );
        println!(
            "哨兵批次确认平均延迟: {:.1}s | P95延迟: {:.1}s\n",
    ...
    if std::env::args().any(|a| a == "--check-integrity") {
        ...
        for s in &symbols {
            let report =
                n_core::integrity::RawDataIntegrityChecker::inspect_symbol(&db, &s.code, 1000)
                    .await?;
    ...
    if config.email.smtp_password.is_empty() && config.email.from.is_empty() {
        for p in [data_dir.join("email.toml"), PathBuf::from("email.toml")] {
            if p.exists() {
                if let Ok(imported) = n_core::config::import_email_toml(&p) {
                    if !imported.smtp_password.is_empty()
                        || imported.from != n_core::notify::email::EmailSettings::default().from
                    {
                        config.email = imported;
                        let _ = config.save(&config_path);
                        tracing::info!("已从 {} 导入邮件配置", p.display());
                        break;
                    }
                }
            }
    ...
    let cnt = n_core::storage::repo::list_symbols(&services.db, false)
        .await
        .map(|v| v.len())
        .unwrap_or(0);
        ...
        loop {
            ...
            match action {
                ...
                n_core::scheduler::SchedulerAction::Refresh => {
                    if run_refresh(&svc).await.is_ok() {
                        last_refresh = Some(now);
                    }
                }
...
async fn run_scan(svc: &Services) -> anyhow::Result<()> {
        ...
        for (kind, e) in emails {
            ...
            if let Err(err) = n_core::notify::email::send_summary(&subject, &body, &cfg.email) {
                ...
                tracing::info!(
                    "已发送邮件 {} {}",
                    e.symbol,
                    if kind == n_core::notify::email::EventEmailKind::Warning {
                        "预警"
                    } else {
                        "触发"
                    }
    ...
    } else if res.has_notifiable_signal(min_score) {
        tracing::warn!("有新信号但邮件未配置");
    }
    Ok(())
}
